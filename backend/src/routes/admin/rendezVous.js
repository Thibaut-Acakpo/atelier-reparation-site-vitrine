...
const { cleanText, isFutureDate, isWithinBusinessHours } = require('../../utils/validation');
...
router.patch('/:id', (req, res, next) => {
  try {
    const existant = db.prepare(`SELECT * FROM rendez_vous WHERE id = ?`).get(req.params.id);
    if (!existant) return fail(res, 'Rendez-vous introuvable.', 404);
    ...
    if (statut === 'modifie') {
      ...
      if (!isFutureDate(dateSouhaitee)) return fail(res, 'Nouvelle date invalide.', 422, ['date_souhaitee']);
      if (!isWithinBusinessHours(dateSouhaitee, heureSouhaitee)) {
        return fail(res, "Nouvelle heure hors des horaires de l'atelier.", 422, ['heure_souhaitee']);
      }
    }

    db.prepare(`UPDATE rendez_vous SET statut = ?, date_souhaitee = ?, heure_souhaitee = ? WHERE id = ?`).run(
      statut,
      dateSouhaitee,
      heureSouhaitee,
      req.params.id
